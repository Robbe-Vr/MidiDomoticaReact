...
import { Authorize, useAccount } from "./api/account";
...
function AppShell() {
    const { authorized, loaded, logout } = useAccount();
    ...
    if (!authorized) {
        return (window.location.pathname == '/signin/login' ?
            <>
                <LogInPage setTitle={setTitle} Api={Api} onSuccess={() => { window.location.href = '/'; }} />
            </> :
            <>
                <NotAuthorized setTitle={setTitle} />
            </>
        )
    }
